...
import { maybeGetSimpleString, getSimpleFragmentName } from "./utils";
...
export class RazorData {
  constructor({ args = null, name = null, type = null, fragmentType = null }) {
    ...
    if (type === "fragment" && !name) throw new Error("fragments must come with a name");
  ...
  print() {
    ...
    if (!fields.length)
      return (
        /* eslint-disable-next-line */
        console.log("babel-blade Warning: razor with no children, doublecheck") || null
      ); // really shouldnt happen, should we throw an error?
    const maybeArgs = coerceNullLiteralToNull(this._args && this._args[0]);
    const TemplateLiteral = appendLiterals();
    if (this._type === "query") {
    ...
    TemplateLiteral.addStr(maybeArgs ? "(" : "")
      ...
      .addStr("{\n");
    const indent = "  ";
    const fragments = []; // will be mutated to add all the fragments included
    const accumulators = Object.keys(fields).map((key) => fields[key].print(indent, fragments));
...
export class BladeData {
  ...
  print(indent, fragments) {
    const maybeArgs = this._args.length && this._args;
    const maybeDirs = this._directives.length && this._directives;
    const alias = this._alias;
    const printName = alias ? `${alias}: ${this._name}` : this._name;
    ...
    if (fields.length || this._fragments.length) {
      TemplateLiteral.addStr(" {\n");
      const accumulators = Object.keys(fields).map((key) =>
        /* eslint-disable-next-line */
...
function appendLiterals() {
  const stringAccumulator = [];
  const litAccumulator = [];
  const me = {
    addStr(str = null) {
...
function zipAccumulators({ stringAccumulator, litAccumulator }) {
  ...
  for (let i = 0; i < stringAccumulator.length; i++) {
    if (litAccumulator[i]) {
      const maybeSimpleString = maybeGetSimpleString(litAccumulator[i]);
