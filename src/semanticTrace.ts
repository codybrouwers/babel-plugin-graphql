...
export function semanticTrace(referencePath, origin, semanticVisitor, semanticPath = []) {
...
function traceRHS(ref, semanticPath, semanticVisitor) {
  ...
  function isValidRHSParent(ptr) {
    const baseLayer = ["Member", "Call"].map((x) => `${x}Expression`).includes(ptr.parentPath.type);
  ...
  function workOnRHSParent(ptr, newSemanticPath, semanticVisitor) {
    ...
    function isValidArrayPrototypeInternal(ptr) {
      ...
      const isArrowChild = ["ArrowFunctionExpression", "FunctionExpression"].includes(
        ptr.parentPath.get("arguments")[0].type
      );
